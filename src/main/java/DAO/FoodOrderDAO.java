...
public class FoodOrderDAO extends DAO {
	...
	public ArrayList<FoodOrder> getFoodOrdered(int idBill) {
		...
		String sql = "SELECT fo.*, mf.name, mf.price AS menuFoodPrice, mf.type, mf.des " + "FROM tblFoodOrder fo "
				+ "JOIN tblMenuFood mf ON fo.tblMenuFoodid = mf.id " + "WHERE fo.tblBillid = ?";
	...
	public boolean updateFoodOrderQuantity(int foodOrderId, int newQuantity) {
		String sql = "UPDATE tblFoodOrder SET quantity = ? WHERE id = ?";
		try (PreparedStatement ps = con.prepareStatement(sql)) {
	...
	public boolean deleteFoodOrder(int foodOrderId) {
		String sql = "DELETE FROM tblFoodOrder WHERE id = ?";
		try (PreparedStatement ps = con.prepareStatement(sql)) {
